...
export const showDownloadsDialog = (user: IUser, browserWindow: BrowserWindow, x: number, y: number): Dialog => {
    ...
    const dynamicDialog = dialogManager.getDynamic(DIALOG_DOWNLOADS_NAME);
    if (dynamicDialog) {
        ...
        dialog.webContents.loadFile(join(app.getAppPath(), 'build', 'browser', 'downloads.html'));
