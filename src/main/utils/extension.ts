...
const calcLength = (a: number, b: number, c: number, d: number) => {
    let length = 0;
    length += a << 0;
    length += b << 8;
    length += c << 16;
...
const getBinaryString = (bytesView: Buffer, startOffset: number, endOffset: number) => {
    let binaryString = '';
...
const getPublicKeyFromProtoBuf = (bytesView: Buffer, startOffset: number, endOffset: number) => {
