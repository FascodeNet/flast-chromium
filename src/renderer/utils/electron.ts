...
const usersApi = {
    list: (): Promise<UserData[]> => ipcRenderer.invoke(IPCChannel.Users.LIST())
} as const;

const userApi = {
    current: (): Promise<string> => ipcRenderer.invoke(`window-user-${windowId}`),
    getType: (userId: string): Promise<UserType> => ipcRenderer.invoke(IPCChannel.User.TYPE(userId)),
    getConfig: (userId: string): Promise<UserConfig> => ipcRenderer.invoke(IPCChannel.User.GET_CONFIG(userId)),
    setConfig: (userId: string, config: DeepPartial<UserConfig>): Promise<UserConfig> => ipcRenderer.invoke(IPCChannel.User.SET_CONFIG(userId), config)
} as const;
...
const popupApi = {
    ...
    profile: (x: number, y: number): Promise<void> => ipcRenderer.invoke(IPCChannel.Popup.PROFILE(windowId), x, y),
