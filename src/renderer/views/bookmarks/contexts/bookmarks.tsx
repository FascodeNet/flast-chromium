...
export const BookmarksProvider = ({ children }: BookmarksProviderProps) => {
    ...
    useEffect(() => {
        window.api.getUser().then(async (id) => {
            ...
            try {
                const bookmarkDataList = await window.api.getBookmarks(id);
    ...
    const addBookmark = async (data: OmitData<BookmarkData>) => {
        try {
            await window.api.addBookmark(userId, data);
    ...
    const removeBookmark = async (id: string) => {
        const result = await window.api.removeBookmark(userId, id);
        if (result)
            await reloadBookmarks();
    };
    ...
    const updateBookmark = async (id: string, data: OmitData<BookmarkData>) => {
        try {
            const result = await window.api.updateBookmark(userId, id, data);
            if (result)
                await reloadBookmarks();
        } catch (e) {
            return;
    ...
    const reloadBookmarks = async () => {
        try {
            const bookmarkDataList = await window.api.getBookmarks(userId);
            setBookmarks(bookmarkDataList);
        } catch (e) {
            return;
        }
